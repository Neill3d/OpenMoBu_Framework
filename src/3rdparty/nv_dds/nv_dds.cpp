...
///////////////////////////////////////////////////////////////////////////////
// loads DDS image from memory
//
// memory - pointer to a memory block whick holds dds image file
// flipImage - specifies whether image is flipped on load, default is true

bool CDDSImage::load(const unsigned char *memory, bool flipImage)
{
    ...
    char filecode[4];
	memcpy(filecode, ptr, sizeof(char) * 4);
	ptr += sizeof(char) * 4;
    ...
    // read in DDS header
    DDS_HEADER *ddsh = (DDS_HEADER*) ptr;
    ...
    for (unsigned int n = 0; n < (unsigned int)(m_type == TextureCubemap ? 6 : 1); n++)
    {
        ...
        // load surface
        const unsigned char *pixels = ptr;
		ptr += size;
...
bool CDDSImage::upload_textureCubemap()
{
    ...
    if (!upload_texture2D(0, target))
		return false;

	target = GL_TEXTURE_CUBE_MAP_NEGATIVE_X;
    if (!upload_texture2D(1, target))
		return false;

	target = GL_TEXTURE_CUBE_MAP_POSITIVE_Y;
    if (!upload_texture2D(2, target))
		return false;

	target = GL_TEXTURE_CUBE_MAP_NEGATIVE_Y;
    if (!upload_texture2D(3, target))
		return false;

	target = GL_TEXTURE_CUBE_MAP_POSITIVE_Z;
    if (!upload_texture2D(4, target))
		return false;

	target = GL_TEXTURE_CUBE_MAP_NEGATIVE_Z;
    if (!upload_texture2D(5, target))
		return false;
