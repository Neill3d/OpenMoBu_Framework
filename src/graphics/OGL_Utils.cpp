...
namespace NOpenMobu 
{
	...
	//constexpr int MIP_LEVEL_COUNT{ 1 }; // UNUSED ?!
	constexpr int COMPOSE_LEVEL_COUNT{ 8 };
	...
	*/
	...
	{
		for (int i = 0; i < COMPOSE_LEVEL_COUNT; ++i)
		{
	...
	bool PrepFrameBufferMS(CFrameBufferMS& fb, const int width, const int height, const int samples, const int flags, const int normalFormat)
	{
		...
		bool useNormal = ((flags & CFrameBufferMS::EFlag::NORMAL) > 0);

		if ((flags & CFrameBufferMS::EFlag::TEXTURES) > 0)
		{
		...
		if ((flags & CFrameBufferMS::EFlag::RENDERBUFFERS) > 0)
		{
		...
		{
			LOG_TRACE("error by prep framebuffer with %d widht %d height %d samples %d flags %d normalFormat!\n", width, height, samples, flags, normalFormat);
			return false;
		}
