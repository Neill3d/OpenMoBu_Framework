...
namespace NOpenMobu 
{
	...
	class GRAPHICS_LIBRARY_API CComputeProgram
	{
	public:
	...
	protected:
		...
		bool loadComputeShader(const char* computeShaderName, const GLuint shaderid, const GLuint programid)
		{
			...
			if (fp)
			{
				...
				memset(&buffer[0], 0, sizeof(char) * (static_cast<size_t>(len) + 1));

				size_t readlen = fread(buffer, sizeof(char), fileLen, fp);

				// trick to zero all outside memory
				memset(&buffer[readlen], 0, sizeof(char) * (static_cast<size_t>(len) + 1 - readlen));
